interface DMessagesAggregatorTestsContext {
  messagesAggregator: dcore._private.DMessagesAggregator;
  topics: string[];
  message: string;
  mockSubscriber: {
...
describe("DMessagesAggregator", () => {
  ...
  beforeEach(function (this: DMessagesAggregatorTestsContext): void {
    ...
    this.messagesAggregator = new dcore._private.DMessagesAggregator();
    this.mockSubscriber = {
  ...
  it("should be able to add subscriber", function (this: DMessagesAggregatorTestsContext, done: DoneFn) {
    ...
    let token = this.messagesAggregator.subscribe(this.topics, this.mockSubscriber.onPublish);
    expect(token).toBeDefined();
    expect(typeof token.destroy).toEqual("function");

    this.topics.forEach(topic => this.messagesAggregator.publish(topic, this.message));
    setTimeout(() => {
